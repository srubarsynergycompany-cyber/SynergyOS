const en = {
  navigation: {
    brand: "SynergyOS",
    subtitle: "AI Fulfillment Platform",
    globalNav: {
      dashboard: "Dashboard",
      orders: "Orders",
      inventory: "Inventory",
      warehouse: "Warehouse",
      packing: "Packing",
      shipping: "Shipping",
      returns: "Returns",
      customers: "Customers",
      products: "Products",
      reports: "Reports",
      settings: "Settings",
    },
    sections: {
      dashboard: "Dashboard",
      orders: "Orders",
      warehouse: "Warehouse",
      crm: "CRM",
      transport: "Transport",
      finance: "Finance",
      ai: "AI Command Center",
    },
    workspaceTitle: "Operations workspace",
    languageLabel: "Language",
    languageSwitcher: {
      cs: "Česky",
      en: "English",
    },
  },
  orders: {
    eyebrow: "Order management",
    title: "Order management",
    subtitle: "A searchable order overview with status handling prepared for future database integration.",
    cta: "+ Create order",
    searchLabel: "Search",
    searchPlaceholder: "Search by order number, customer, or shop",
    statusLabel: "Status",
    statuses: {
      all: "All",
      new: "New",
      picking: "Picking",
      packed: "Packed",
      shipped: "Shipped",
      delivered: "Delivered",
    },
    table: {
      order: "Order",
      customer: "Customer",
      status: "Status",
      carrier: "Carrier",
      items: "Items",
      total: "Total",
      view: "View",
    },
    pagination: {
      showing: "Showing",
      of: "of",
      previous: "Previous",
      next: "Next",
    },
    detail: {
      eyebrow: "Order details",
      subtitle: "Operational and logistics overview for this shipment.",
      back: "Back to orders",
      summary: "Order summary",
      customer: "Customer",
      address: "Delivery address",
      carrier: "Carrier",
      promiseDate: "Promise date",
      notesLabel: "Notes",
      created: "Created",
      salesChannel: "Sales channel",
      paymentStatus: "Payment status",
      priority: "Priority",
      valueLabels: {
        status: {
          new: "New",
          ready_for_picking: "Ready for picking",
          picking: "Picking",
          packed: "Packed",
          ready_to_ship: "Ready to ship",
          shipped: "Shipped",
          cancelled: "Cancelled",
        },
        priority: {
          high: "High",
          normal: "Normal",
          low: "Low",
        },
        paymentStatus: {
          paid: "Paid",
          unpaid: "Unpaid",
          pending: "Pending",
          awaiting: "Awaiting",
        },
      },
      printLabel: "Print label",
      printInvoice: "Print invoice",
      operations: {
        title: "Operations",
        startPicking: "Start Picking",
        markPacked: "Mark Packed",
        markShipped: "Mark Shipped",
        markDelivered: "Mark Delivered",
        completed: "Order completed",
        printLabel: "Print Label",
        printInvoice: "Print Invoice",
      },
      workflow: {
        picker: "Picker",
        pickedAt: "Picked at",
        packedAt: "Packed at",
        shippedAt: "Shipped at",
      },
      customerCard: {
        title: "Customer & delivery",
        name: "Customer",
        contact: "Contact",
        billing: "Billing address",
        shipping: "Shipping address",
      },
      products: {
        title: "Products & stock",
        image: "Image",
        sku: "SKU",
        name: "Product",
        quantity: "Qty",
        location: "Location",
        availability: "Availability",
        inStock: "In stock",
        outOfStock: "Backorder",
      },
      notes: {
        title: "Internal notes",
        placeholder: "Add a warehouse or carrier note…",
        save: "Save note",
      },
      timeline: {
        title: "Timeline",
        created: "Order created",
        updated: "Last updated",
        payment: "Payment received",
        picking: "Picking started",
        packed: "Packed",
        shipped: "Shipped",
        done: "Completed",
        pending: "Pending",
      },
      warehouse: {
        title: "Warehouse actions",
        startPicking: "Start picking",
        markPacked: "Mark as packed",
        markShipped: "Mark as shipped",
      },
      fulfillment: {
        title: "Fulfillment details",
        items: "Item count",
        total: "Order value",
        slot: "Warehouse slot",
        tracking: "Tracking",
      },
    },
  },
  dashboard: {
    eyebrow: "Fulfillment command center",
    title: "Operational overview",
    subtitle: "A clear view of orders, throughput, and carrier health across the network.",
    cta: "+ Create fulfillment run",
    kpis: {
      ordersToday: {
        label: "Orders today",
        change: "+12% vs yesterday",
      },
      readyToPack: {
        label: "Ready to pack",
        change: "6 urgent",
      },
      shippedToday: {
        label: "Shipped today",
        change: "+8% efficiency",
      },
      returns: {
        label: "Returns",
        change: "1 pending review",
      },
    },
    revenue: {
      title: "Revenue trend",
      subtitle: "Last 7 days",
      change: "+18.4% MoM",
    },
    occupancy: {
      title: "Warehouse occupancy",
      subtitle: "Current capacity",
      badge: "Healthy",
      label: "2,340 / 3,000 slots",
      description: "Picking lanes are running at 78% capacity.",
      peak: "Peak window",
      peakDescription: "11:00–14:00 with 2 extra pickers recommended.",
    },
    ordersOverTime: {
      title: "Orders over time",
      subtitle: "Daily throughput",
    },
    carrierStats: {
      title: "Carrier stats",
      subtitle: "Preferred shipping mix",
    },
    topCustomers: {
      title: "Top customers",
      subtitle: "Highest revenue contributors",
      action: "This quarter",
    },
    recentOrders: {
      title: "Recent orders",
      subtitle: "Latest fulfillment activity",
    },
    quickActions: {
      title: "Quick actions",
      subtitle: "Accelerate daily ops",
      batchLabels: {
        title: "Batch print labels",
        description: "Print 24 pending labels in one pass",
      },
      reassignCarrier: {
        title: "Reassign carrier",
        description: "Optimize late shipments to a faster route",
      },
      approveReturns: {
        title: "Approve returns",
        description: "Review three exceptions before close of day",
      },
    },
    notifications: {
      title: "Notifications",
      subtitle: "Team updates",
      items: [
        {
          title: "Inventory alert",
          text: "4 SKUs below reorder point",
          time: "12 min ago",
        },
        {
          title: "Carrier delay",
          text: "DPD hub congestion in Prague",
          time: "34 min ago",
        },
        {
          title: "New return",
          text: "One order requires quality review",
          time: "1h ago",
        },
      ],
    },
    placeholder: {
      title: "This module is ready for the next step in the SynergyOS product roadmap.",
    },
    chartLabels: {
      mon: "Mon",
      tue: "Tue",
      wed: "Wed",
      thu: "Thu",
      fri: "Fri",
      sat: "Sat",
      sun: "Sun",
    },
    table: {
      customer: "Customer",
      revenue: "Revenue",
      orders: "Orders",
      tier: "Tier",
      order: "Order",
      carrier: "Carrier",
      status: "Status",
      time: "Time",
    },
    orderStatuses: {
      packed: "Packed",
      inTransit: "In transit",
      ready: "Ready",
      queued: "Queued",
    },
  },
  modules: {
    modulePage: {
      badge: "Foundation module",
      readySuffix: "module is ready",
      readyDescription: "This section now uses the global app shell, design system, and shared service layer.",
      goToDashboard: "Go to Dashboard",
    },
    inventory: {
      eyebrow: "Inventory module",
      title: "Inventory overview",
      subtitle: "Monitor stock health, reserved quantities, and movement activity across warehouse locations.",
      toolbar: {
        newProduct: "New Product",
        stockIn: "Stock In",
        stockOut: "Stock Out",
        transfer: "Transfer",
        history: "Inventory History",
        adjustStock: "Adjust stock",
      },
      filters: {
        searchLabel: "Search inventory",
        searchPlaceholder: "Search by name, SKU, category, or location",
        barcodeLabel: "Barcode search",
        barcodePlaceholder: "Scan or enter barcode",
      },
      stats: {
        warehouseLocations: "Warehouse locations",
        lowStockItems: "Low stock items",
        reservedToday: "Reserved today",
      },
      table: {
        product: "Product",
        location: "Location",
        current: "Current",
        reserved: "Reserved",
        available: "Available",
        minimum: "Minimum",
        status: "Status",
      },
      statuses: {
        inStock: "In stock",
        lowStock: "Low stock",
        critical: "Critical",
        reserved: "Reserved",
      },
      adjustment: {
        title: "Adjust stock",
        description: "Safely change the quantity at a specific inventory location.",
        product: "Product",
        selectProduct: "Select a product",
        location: "Location",
        selectLocation: "Select a location",
        currentQuantity: "Current quantity",
        delta: "Quantity change",
        deltaPlaceholder: "For example 10 or -3",
        reason: "Adjustment reason",
        reasonPlaceholder: "Describe why the stock is changing",
        resultingQuantity: "Resulting quantity",
        cancel: "Cancel",
        submit: "Save adjustment",
        submitting: "Saving…",
        noInventory: "Product has no inventory row",
        success: "Stock was adjusted successfully.",
        errors: {
          productRequired: "Select a product.",
          locationRequired: "Select a location.",
          deltaInteger: "The change must be an integer.",
          deltaNonZero: "The change cannot be zero.",
          negativeResult: "The resulting quantity cannot be negative.",
          reasonRequired: "Enter an adjustment reason.",
          requestFailed: "The stock adjustment could not be saved.",
        },
      },
      movements: {
        title: "Inventory history",
        description: "Review the audit history of manual stock adjustments.",
        close: "Close",
        loading: "Loading inventory history…",
        refreshing: "Refreshing history…",
        retry: "Try again",
        loadFailed: "Inventory history could not be loaded.",
        emptyTitle: "No movements",
        emptyDescription: "No inventory movements match the selected filters.",
        unknownActor: "Unknown user",
        deletedProduct: "Deleted product",
        filters: {
          sku: "SKU",
          skuPlaceholder: "Search by SKU",
          product: "Product",
          productPlaceholder: "Search by product name",
          location: "Location",
          locationPlaceholder: "Search by location",
          from: "Date from",
          to: "Date to",
          apply: "Apply filters",
          clear: "Clear filters",
          invalidRange: "Date from must not be later than date to.",
        },
        table: {
          createdAt: "Date and time",
          sku: "SKU",
          product: "Product",
          location: "Location",
          quantityBefore: "Before",
          delta: "Change",
          quantityAfter: "After",
          reason: "Reason",
          actor: "Actor",
        },
        pagination: {
          previous: "Previous",
          next: "Next",
          page: "Page",
          of: "of",
          results: "results",
        },
        dateLocale: "en-US",
      },
    },
    warehouse: {
      eyebrow: "Warehouse operations",
      title: "Warehouse control center",
      subtitle: "Coordinate receiving, transfers, replenishment, and cycle counts from one streamlined workspace.",
      links: {
        receiving: "Receiving",
        transfers: "Transfers",
      },
      metrics: {
        activePickers: "Active pickers",
        activePickersDetail: "2 on route",
        ordersWaiting: "Orders waiting",
        ordersWaitingDetail: "4 priority",
        receivingTasks: "Receiving tasks",
        receivingTasksDetail: "1 ready to confirm",
        transferTasks: "Transfer tasks",
        transferTasksDetail: "2 awaiting scan",
        inventoryAlerts: "Inventory alerts",
        inventoryAlertsDetail: "3 critical",
        lowStockAlerts: "Low stock alerts",
        lowStockAlertsDetail: "Pick locations",
      },
      sections: {
        operationalFocus: "Operational focus",
        focus1: "Receive purchase orders and confirm locations in a single flow.",
        focus2: "Track transfer tasks and scan-barcode confirmation for fast movement between zones.",
        focus3: "Replenish low picking locations and review cycle-count differences before approvals.",
        quickActions: "Quick actions",
        receivePo: "Receive PO",
        startTransfer: "Start transfer",
        replenish: "Replenish",
        countStock: "Count stock",
      },
      receiving: {
        eyebrow: "Receiving",
        title: "Manual goods receiving",
        subtitle: "Receive one product into an existing warehouse location and immediately update the real stock.",
        loading: "Loading inventory…",
        refreshing: "Refreshing inventory…",
        retry: "Try again",
        formTitle: "New receipt",
        formDescription: "Select a product and its existing location, then confirm the received quantity.",
        product: "Product",
        selectProduct: "Select a product",
        location: "Destination location",
        selectLocation: "Select a location",
        receivedQuantity: "Received quantity",
        quantityPlaceholder: "For example 10",
        reference: "Delivery or receipt reference",
        referencePlaceholder: "For example REF-2026-001",
        note: "Note (optional)",
        notePlaceholder: "For example received without damage",
        currentQuantity: "Current quantity",
        resultingQuantity: "Resulting quantity",
        previewTitle: "Stock preview",
        previewDescription: "Review the effect of this receipt before confirming it.",
        submit: "Confirm receipt",
        submitting: "Receiving…",
        reasonPrefix: "Receiving",
        empty: {
          title: "No inventory items",
          description: "There are no existing inventory rows available for manual receiving.",
        },
        success: {
          title: "Receipt confirmed successfully",
          description: "Stock at the selected location has been updated.",
        },
        errors: {
          loadFailed: "Inventory could not be loaded.",
          invalidInventory: "The server returned invalid inventory data.",
          productRequired: "Select a product.",
          locationRequired: "Select a destination location.",
          quantityPositiveInteger: "Received quantity must be a positive integer.",
          referenceRequired: "Enter a delivery or receipt reference.",
          submitFailed: "The receipt could not be confirmed.",
          invalidResponse: "The server did not return a valid receipt confirmation.",
        },
      },
    },
    packing: {
      title: "Packing",
      description: "Pack station orchestration, quality checks, and parcel readiness.",
    },
    products: {
      title: "Products",
      description: "Catalog governance, SKU attributes, and product lifecycle.",
    },
    productsModule: {
      eyebrow: "Products module",
      title: "Products catalog",
      subtitle: "Manage SKU master data with stock constraints and customer mappings.",
      createButton: "Create product",
      updateButton: "Save changes",
      deleteButton: "Delete",
      detailTitle: "Product detail",
      searchLabel: "Search",
      searchPlaceholder: "Search by SKU, EAN, name, customer, or category",
      categoryFilter: "Category",
      categoryAll: "All categories",
      activeFilter: "Status",
      activeAll: "All",
      activeOnly: "Active only",
      inactiveOnly: "Inactive only",
      paginationShowing: "Showing",
      paginationOf: "of",
      paginationPrev: "Previous",
      paginationNext: "Next",
      emptyTitle: "No products found",
      emptyDescription: "Try clearing filters or create a new product.",
      validation: {
        skuRequired: "SKU is required.",
        eanRequired: "EAN barcode is required.",
        nameRequired: "Name is required.",
        descriptionRequired: "Description is required.",
        customerRequired: "Customer is required.",
        categoryRequired: "Category is required.",
        unitRequired: "Unit is required.",
      },
      table: {
        sku: "SKU",
        ean: "EAN",
        name: "Name",
        customer: "Customer",
        category: "Category",
        stock: "Stock",
        active: "Active",
        actions: "Actions",
        edit: "Edit",
        detail: "Detail",
      },
      form: {
        createTitle: "Create product",
        editTitle: "Edit product",
        sku: "SKU",
        eanBarcode: "EAN barcode",
        name: "Name",
        description: "Description",
        customer: "Customer",
        category: "Category",
        weight: "Weight",
        width: "Width",
        height: "Height",
        length: "Length",
        minimumStock: "Minimum stock",
        currentStock: "Current stock",
        unit: "Unit",
        active: "Active",
        save: "Save",
        cancel: "Cancel",
      },
      detail: {
        dimensions: "Dimensions",
        volumeHint: "L x W x H",
        stock: "Stock",
        minimumStock: "Minimum stock",
        unit: "Unit",
        active: "Active",
        createdAt: "Created",
        updatedAt: "Updated",
        futureTitle: "Prepared for future",
        warehousePositions: "Warehouse positions",
        batches: "Batches",
        expirationDates: "Expiration dates",
        images: "Images",
        attachments: "Attachments",
      },
    },
    reports: {
      title: "Reports",
      description: "Operational KPIs, trend analysis, and decision dashboards.",
    },
    returns: {
      title: "Returns",
      description: "Reverse logistics intake, inspections, and disposition paths.",
    },
    customers: {
      title: "Customers",
      description: "Account health, SLA visibility, and customer operations.",
    },
    shipping: {
      title: "Shipping",
      description: "Carrier labels, dispatch windows, and handoff tracking.",
    },
    settings: {
      title: "Settings",
      subtitle: "Global defaults for operations, notifications, and profile preferences.",
      badge: "Configuration",
      form: {
        title: "Workspace profile",
        description: "Example form primitives from the global design system.",
        workspaceName: "Workspace name",
        workspaceNamePlaceholder: "Synergy Fulfillment",
        defaultLocale: "Default locale",
        defaultLocalePlaceholder: "en-US",
        operationsEmail: "Operations email",
        operationsEmailHint: "Used for system alerts and escalations.",
        operationsEmailPlaceholder: "ops@synergyos.com",
        dailyCutoffTime: "Daily cut-off time",
        saveChanges: "Save changes",
        cancel: "Cancel",
      },
    },
  },
};

export default en;
